...
public class BBSServiceImp implements BBSService {
	 ...
	 * 2번이상의 SQL문을 Transaction이라는 작업그룹으로 묶어 주어야 한다.
	 * 
	 * 스프링과 mybatis 프로젝트에서는
	 * mybatis-context.xml 또는 , MybatisConfig 에서
	 * DataTransactionManager를 bean으로 설정 해 두고
	 * 
	 * transaction이 필요한 method에서
	 * @Transactional을 설정 해 두면
	 * 나머지는 Spring, Mybatis, DataSource 들 끼리 서로 협력하여
	 * 모든 처리를 대신 해 준다.
	 * 
	 * Config에서 transactionManager 라는 이름으로 method를 설정 해야 하고
	 * Config.java로 설정할때는 @EnableTransactionManager를 설정한다.
	 * 
	 * MySQL은 기본 DB엔진에서는 자체적으로 Transaction을 지원하지 않는다.
	 * 그래서 MySQL을 처음 설치할때 DB 엔진을 innoDB로 설정을 해야한다.
	 * 
