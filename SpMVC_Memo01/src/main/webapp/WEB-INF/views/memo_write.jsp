...
function myfunc(){
	//alert("반갑습니다.")
	/*	
 	document(HTML문서)에서 Id가 m_menu로 설정된 tag를 찾아서
 	그곳의 문자열을 반갑습니다로 설정하라 
 	*/
	document.getElementById("m_message").innerHTML = "반갑습니다"
	
	/*
	var : JS에서 변수를 선언하는 key word
	String, int 이런 형들은 없이 var, let이라는 keyword로 시작되면
	변수 선언문이 된다.
	*/
	...
	/*
	form에 있는 메모저장 button의 click을 무력화 시키고 JS함수를
	실행하도록 설정을 변경했기 때문에 form에 입력된 데이터들을
	서버로 전송하기 위해 마지막에 submit을 강제로 실행 시켜 줘야 한다.
	*/
...
<section>
	...
	placeholder="작성자 이름을 입력하세요" class="w3-input w3-border w3-hover-light-gray">
	
	<label for="m_date">작성일자</label>
	<input type="text" id="m_date" name="m_date" 
	placeholder="작성 일자를 입력하세요" class="w3-input w3-border w3-hover-light-gray">
	
	<label for="m_subject">제목</label>
	<input type="text" id="m_subject" name="m_subject" 
	placeholder="제목을 입력하세요" class="w3-input w3-border w3-hover-light-gray">
	
	<label>메모</label>
	<textarea rows="15" id="m_text" name="m_text" 
	placeholder="메모 내용을 입력하세요" class="w3-input w3-border w3-hover-light-gray"></textarea>
	
	<button onclick="myfunc()" type="button" class="w3-button w3-purple">메모저장</button>
	
	<span id="m_message">메시지 영역</span>
	</form>
</section>
</body>
</html>