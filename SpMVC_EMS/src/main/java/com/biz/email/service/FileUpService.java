...
public class FileUpService {
	...
	public String upload(MultipartFile file) {
		...
		// 경로를 추출하고, 파일을 저장할 폴더(/files/)가 만들어 졌는지 확인하고
		// 없으면 생성을 해 주어야 한다.
		File dir = new File(realPath); // 운영체제한테 /files/ 정보를 달라고 요청
		if(!dir.exists()) // 만약 /files/ 에 대한 정보가 없으면, 디렉토리가 없으면
			dir.mkdir(); // 생성하라
			// dir.mkdirs() // 다중경로 디렉토리 생성
		
		// 업로드한 파일에 여러가지 문제가 발생하면 더이상 진행X
		if(file.isEmpty() || file == null) return null;

		// 업로드할 파일의 이름을 추출
		String realFile = file.getOriginalFilename();
		// 원래의 파일이름으로 파일을 업로드 할 경우
		// 같은 이름이 있을 경우 미리 앞에 업로드된 파일이 나중파일로
		// 교체되버리는 현상이 있어서
		// 실제로 파일을 업로드 할때는 임의의 이름으로 변경해주는 것이 좋다.
		String saveFile = UUID.randomUUID().toString(); // 32자리의 16진수 문자열로 된 임의 값
		saveFile += realFile ;
