...
public class MenuService {
	...
	public List<MenuDto> getBuilder() {
		...
		// 생성자 패턴 
		//		다량의 데이터를 setting 해야 할 경우
		//		생성자에서 직접 데이터를 주입하면서, dto(vo)를 만들수 있다.
		//		단점 : 데이터를 주입할때 항상 순서에 신경을 써야 한다.
		// 				만약 순서를 제대로 지키지 않으면 데이터가 엉뚱하게 사용될 수 있다.
		// 생성자 패턴의 그러한 단점을 보완하기 위해서
		// setter 패턴을 사용한다.
		//		setter 패턴의 가장 큰 단점
		//		일단 코드가 길어지고, 난해한(읽기 힘든) 코드가 될 수 있다.
		
		// Builder 패턴은 생성자 호출을하면서
		// 변수이름을 직접 보고 데이터를 추가할 수 있기 때문에
		// 순서가 바뀌거나 하는 경우가 없고.
