		...
		<c:choose>
			...
			<tr>
				...
				<c:if test="${VO.io_check=='1'}">
					<td>${VO.io_price}</td>
					<td>
						<fmt:formatNumber value="${VO.io_price * VO.io_su}" type="number" />
					</td>
					<td></td><td></td>
				</c:if>
				<!-- 매출표시 -->
				<c:if test="${VO.io_check=='2'}">
					...
					<td>${VO.io_price}</td>
					<td>
						<fmt:formatNumber value="${VO.io_price * VO.io_su}" />
					</td>
				</c:if>
			</tr>
			</c:forEach>	
...
<!-- 
	...
	fmt formatNumber var="값.값" type="percent" 만약0.3이면 30%표현 1.2면 120% 표현
	
	fmt formatNumber var="값" pattern=".00" 숫자형으로 표현하되 소수점이하 2자리까지 표시
 -->